...
my $numberOfMethods=6;

my $DirName="Complicated-AlltoAll-datamining-$subFlowNum"."SF-$failureNumber-$load-$switchDownPortNumber-$linkRate";
...
my @methods=("RMPTCP-RETRAN","ECMP","MPTCP","REACTIVE","CORRECTIVE","REPFLOW");
...
for(my $i=0;$i<$numberOfMethods;$i++)
{
	...
	`yes|cp RMT-FatTree-warmup.tcl $DirName/$str`;
}
...
for (my $runTime = 0; $runTime < $TimeToRun; $runTime++) 
{
	...
	for(my $i=0;$i<$numberOfMethods;$i++)
	{
		...
		`yes|cp $curdir/$DirName/$str/RMT-FatTree-warmup.tcl $curdir/$DirName/$str/$str-$runTime`;
	    ...
	    unless ($pid) {
	        ...
	        if($str ne "Ideal")
	        {
		    	print "ns RMT-FatTree-warmup.tcl Sack 400 $linkDelay $flowNumber $switchPortNumber $linkRate $switchDownPortNumber $linkAccessRate $switchLayer $str $failureNumber $subFlowNum\n";
		    	system("ns RMT-FatTree-warmup.tcl Sack 400 $linkDelay $flowNumber $switchPortNumber $linkRate $switchDownPortNumber $linkAccessRate $switchLayer $str $failureNumber $subFlowNum 1>debug.tr 2>stderr.tr") == 0
				or do {
			...
			{
				print "ns RMT-FatTree-warmup.tcl Sack 400 $linkDelay $flowNumber $switchPortNumber $linkRate $switchDownPortNumber $linkAccessRate $switchLayer ECMP 0 1\n";
		    	system("ns RMT-FatTree-warmup.tcl Sack 400 $linkDelay $flowNumber $switchPortNumber $linkRate $switchDownPortNumber $linkAccessRate $switchLayer ECMP 0 1 1>debug.tr 2>stderr.tr") == 0
				or do {
					print "ERROR: $str runTime=$runTime failed\n";
					print fError "$str runTime=$runTime failed\n";
				};
