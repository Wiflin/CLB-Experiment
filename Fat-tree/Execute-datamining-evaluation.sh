...
while [ $(bc <<< "$load <= $MAX_LOAD") -eq 1 ]; do
	...
	for ((  j = 1 ;  j <= $CON_LOAD_NUM;  j++  )); do
		if [[ $(bc <<< "$load + $LOAD_STEP <= $MAX_LOAD") -eq 1  && $j < $CON_LOAD_NUM ]]; then
			echo "Complicated-AlltoAll-datamining-RMT-FatTree-long-connection.pl $load $randSeed &"
			perl Complicated-AlltoAll-datamining-RMT-FatTree-long-connection.pl $load $randSeed &
