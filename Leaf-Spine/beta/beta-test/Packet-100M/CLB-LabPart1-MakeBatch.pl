...
my $linkDelay=0.02;
my $switchAccessNumber=2;
my $linkRate='10G';
my $switchDownPortNumber=40;
my $linkAccessRate='1G';
# my $switchLayer=3;
...
print "linkDelay=$linkDelay(ms), linkRate=$linkRate, switchAccessNumber=$switchAccessNumber, switchDownPortNumber=$switchDownPortNumber, linkAccessRate=$linkAccessRate, flowPerHost=$flowPerHost, subFlowNum=$subFlowNum\n";
print "load=$load,randseed=$ARGV[1]\n";
srand("$ARGV[1]");


my $hostInTotal=$switchAccessNumber*$switchDownPortNumber;
...
my $DirName="Long-connection-websearch-$subFlowNum"."SF-$load-$switchDownPortNumber-$linkRate";
...
print fResultEachNormalFCT "FlowID\tFlowSize\n";
...
for(my $i=0;$i<$serverPerBiNetwork;$i++)
{
	...
	#	fTraffic->print("$i $dst[$i][$z] $startTime[$i] $size[$i][$z]\n");
		fTraffic->print("$i $dst[$i][$z] 1.0000000 100000000\n");
