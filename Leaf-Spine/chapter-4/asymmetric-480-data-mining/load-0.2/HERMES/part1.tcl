...
set queueSpineSwitch 2400
set queueLeafSwitch 2400
...
set downLinkSwitch	2
set downLinkRate 	40G
set downLinkQueue	1600
# 0.02 ms
set linkDelay 	0.02 
# 
...
for {set i 0} {$i<$downLinkSwitch} {incr i} {
...
for {set i $downLinkSwitch} {$i<$accessSwitchNumber} {incr i} {
	for {set j 0} {$j<$leafDownPortNumber} {incr j} {
		set k [expr $i*$leafDownPortNumber+$j]
		$ns simplex-link $n($k) $sLeaf($i) $downLinkRate 0.00002s $queueManage
		$ns queue-limit $n($k) $sLeaf($i) $downLinkQueue
		$ns ecn-threshold $n($k) $sLeaf($i) [expr int($downLinkQueue*$ecnThresholdPortionLeaf)]

		$ns simplex-link $sLeaf($i) $n($k) $downLinkRate 0.00002s $queueManage
		$ns queue-limit $sLeaf($i) $n($k) $downLinkQueue
		$ns ecn-threshold $sLeaf($i) $n($k) [expr int($downLinkQueue*$ecnThresholdPortionLeaf)]
		puts "node([$n($k) set address_]) <-> leaf($i)([$sLeaf($i) set address_]) $downLinkRate "
...
$ns queue-limit $sLeaf(1) $sSpine(0) 400
$ns ecn-threshold $sLeaf(1) $sSpine(0) [expr int(400*$ecnThresholdPortion)]
$ns simplex-link $sSpine(0) $sLeaf(1) 10G 0.00002s $queueManage
$ns queue-limit $sSpine(0) $sLeaf(1) 400
$ns ecn-threshold $sSpine(0) $sLeaf(1) [expr int(400*$ecnThresholdPortion)]
