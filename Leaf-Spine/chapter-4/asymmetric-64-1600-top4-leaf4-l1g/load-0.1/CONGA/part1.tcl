...
set linkRate	4G
set queueSpineSwitch 160
set queueLeafSwitch 160
set ecnThresholdPortion 0.3

set linkAccessRate 	1G
set sendBufferSize	40
set ecnThresholdPortionLeaf 0.3


set faultAccessNum	1
set faultAccessPort 1
set faultLinkRate 	1G
set faultLinkBufferSize	40
...
for {set i 0} {$i<$accessSwitchNumber} {incr i} {
	for {set j 0} {$j<$leafUpPortNumber} {incr j} {
		...
		if { $i == $faultAccessNum && $k <= $faultAccessPort } {
			
			$ns simplex-link $sLeaf($i) $sSpine($k) $faultLinkRate [expr $linkDelay*1E-3]s $queueManage	
			$ns queue-limit $sLeaf($i) $sSpine($k) $faultLinkBufferSize
			$ns ecn-threshold $sLeaf($i) $sSpine($k) [expr int($faultLinkBufferSize*$ecnThresholdPortion)]
			$ns simplex-link $sSpine($k) $sLeaf($i) $faultLinkRate [expr $linkDelay*1E-3]s $queueManage
			$ns queue-limit $sSpine($k) $sLeaf($i) $faultLinkBufferSize
			$ns ecn-threshold $sSpine($k) $sLeaf($i) [expr int($faultLinkBufferSize*$ecnThresholdPortion)]

			puts "leaf($i)([$sLeaf($i) set address_]) <-> spine($k)([$sSpine($k) set address_])  rate: $faultLinkRate" 
			...
			puts "leaf($i)([$sLeaf($i) set address_]) <-> spine($k)([$sSpine($k) set address_])  rate: $linkRate"
...
for {set k 0} {$k<$flowNumber} {incr k} {
	...
	# if {$flowSize>$NonLongConnFlowThreshold} {
		...
		$tcp($currentInsertingConnID) ecmp-hash-key [expr int([$rnd value]*0x10000)]
...
set timeLength [expr $latestFlowStartTime+100]
