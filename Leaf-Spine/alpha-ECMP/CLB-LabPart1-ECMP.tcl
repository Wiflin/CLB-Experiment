...
set accessSwitchNumber 	[expr $switchPortNumber]
set serverNumber 		[expr $accessSwitchNumber*$switchDownPortNumber]
set topSwitchNumber 	[expr int($switchPortNumber/2)]
...
for {set i 0} {$i<$accessSwitchNumber} {incr i} {
	for {set j 0} {$j<$leafUpPortNumber} {incr j} {
		set k [expr ($i*$leafUpPortNumber+$j)%$topSwitchNumber]
			...
			# puts "leaf([expr $switchLayer-2],$i)([$sLeaf($i) set address_]) <-> spine($k)([$sSpine($k) set address_])"
