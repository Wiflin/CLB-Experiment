...
void CLBTunnelAgent::createParityPackets(Classifier* ParentClassifier, Handler* hdl)
{
	...
	PrintPacket(parityPacket,"Send parity");
...
void CLBTunnelAgent::encode(Classifier* ParentClassifier, Packet* pkt, Handler* hdl)
{
	...
	{
		// printf("%lf-NODE %d: Free EncodeBuffer! BlockSize_K=%d\n",Scheduler::instance().clock(),ParentClassifier_->nodeID(),BlockSize_K);
		Tcl& tcl = Tcl::instance();
	...
	if(encodeBuffer_.capacity==encodeBuffer_.length)////Encode buffer full, shift it
	{
...
void CLBTunnelAgent::insertReorderBuffer(Packet* pkt)
{
	...
	if(cmnh->ifParity==0)
	{
		...
		> MAX_GENERATION/2)
		{
			PrintPacket(pkt,"Hand to OS out-dated");
			targetRecv(pkt,0);
			return;
		}
		...
		if(reorderBuffer_.buffer[insertPos]!=NULL 
			&& (HDR_CMN(reorderBuffer_.buffer[insertPos]))->ifParity!=-1)
		{
			PrintPacket(pkt,"Receive",1);
		...
		if(((cmnh->genIndex - parityBuffer_[headReorderBuffer].headIndex+MAX_PACKETS_COUNT)%MAX_PACKETS_COUNT)
				< MAX_PACKETS_COUNT/2)
		{
			if(parityBuffer_[headReorderBuffer].ifUpdated==0)
			{
				if((cmnh->genIndex - expectHandOverGenIndex + MAX_PACKETS_COUNT)%MAX_PACKETS_COUNT <= parityBuffer_[headReorderBuffer].blockSize)
				{
		...
		if(parityBuffer_[headReorderBuffer].count > parityBuffer_[headReorderBuffer].blockSize
			&& parityBuffer_[headReorderBuffer].ifUpdated==0)
		{
			// printf("%lf-NODE %d: reorderBuffer_ shift!\n",Scheduler::instance().clock(),ParentClassifier_->nodeID());
			parityBuffer_[headReorderBuffer].headIndex
			=(reorderBuffer_.headIndex+parityBuffer_[headReorderBuffer].count-parityBuffer_[headReorderBuffer].blockSize)%MAX_PACKETS_COUNT;
			parityBuffer_[headReorderBuffer].lastGenOriginalIndex
			=(parityBuffer_[headReorderBuffer].headIndex+BlockSize_MaxK-1)%MAX_PACKETS_COUNT;

			int nextBuff=(headReorderBuffer+1)%ReorderGenNumber;
			if(parityBuffer_[nextBuff].ifUpdated==1)
			{
				int diff=
				(parityBuffer_[headReorderBuffer].headIndex-parityBuffer_[nextBuff].headIndex+MAX_PACKETS_COUNT)%MAX_PACKETS_COUNT;
				if(diff<MAX_PACKETS_COUNT/2)
				{
					clearParityBuffer(headReorderBuffer);
					handOverAndClearBeforeHead(headReorderBuffer);
					return;
				}
			}

			updateParityBuffInfo(headReorderBuffer);
			handOverAndClearBeforeHead(headReorderBuffer);
			parityBuffer_[headReorderBuffer].count=parityBuffer_[headReorderBuffer].blockSize;
		}	
	}
	else ////Parity
	{
		...
		if((cmnh->headGenIndex - reorderBuffer_.headIndex + MAX_GENERATION) % MAX_GENERATION
		> MAX_GENERATION/2)
		{
			// if(cmnh->genID!=parityBuffer_[headReorderBuffer].genID)
			// {
			// 	printf("ERROR: Drop error parity!\n");
			// 	PrintPacket(pkt,"Receive",1);
			// 	PrintReorderBuffer(1);
			// 	exit(1);
			// }
			...
			// clearParityBuffer(headReorderBuffer);
			// handOverAndClearBeforeHead(headReorderBuffer);
			if(whichBufferShouldIn==headReorderBuffer
				&& parityBuffer_[whichBufferShouldIn].ifUpdated==0)
			{
				clearParityBuffer(headReorderBuffer);
				updateParityBuffInfo(headReorderBuffer);
				handOverAndClearBeforeHead(headReorderBuffer);
		...
		if(parityBuffer_[whichBufferShouldIn].ifUpdated==0)
		{
			...
			updateParityBuffInfo(whichBufferShouldIn);

			if(whichBufferShouldIn==headReorderBuffer)
			{
				handOverAndClearBeforeHead(whichBufferShouldIn);
			}
		...
		if((cmnh->genID - parityBuffer_[headReorderBuffer].genID + MAX_GENERATION) % MAX_GENERATION >= 2)
		{ 
			// printf("%lf-NODE %d (Flow %d): The third gen come, handOverAndClearGen(%d)!\n"
			// 	,Scheduler::instance().clock()
		...
		>=parityBuffer_[headReorderBuffer].blockSize)
		{
			...
			{
				parityBuffer_[headReorderBuffer].headIndex
				=(expectHandOverGenIndex-parityBuffer_[headReorderBuffer].blockSize+MAX_PACKETS_COUNT)%MAX_PACKETS_COUNT;
				parityBuffer_[headReorderBuffer].blockSize=BlockSize_MaxK;
				parityBuffer_[headReorderBuffer].lastGenOriginalIndex
				=(parityBuffer_[headReorderBuffer].headIndex+BlockSize_MaxK-1)%MAX_PACKETS_COUNT;
				updateParityBuffInfo(headReorderBuffer);
				handOverAndClearBeforeHead(headReorderBuffer);
...
void CLBTunnelAgent::checkReorderBuffer(Classifier* ParentClassifier, Handler* hdl)
{
	...
	if(cmnh->ifParity==0 || cmnh->ifParity==-1)
	{
		...
		if(parityBuffer_[buffID].count>=parityBuffer_[buffID].blockSize)
		{
			...
			for(int i=0;i<parityBuffer_[buffID].blockSize;i++)
			{
				...
				if(reorderBuffer_.buffer[pos]==NULL)
				{
					...
					(HDR_CMN(reorderBuffer_.buffer[pos]))->ifParity=-1;
					reorderBuffer_.count++;
					packetsUnHandedInReorderBuffer++;
				}
			}
...
void CLBTunnelAgent::handOverAndClearBeforeHead(int bufferID)
{
	
	if(bufferID!=headReorderBuffer)
	{
		printf("ERROR---NODE %d: Should only handOverAndClearBeforeHead(%d) Head!\n"
			,ParentClassifier_->nodeID(),bufferID);
		PrintReorderBuffer(1);
		exit(1);
	}
	else
	{
		int clearPackets=(parityBuffer_[bufferID].headIndex
		-reorderBuffer_.headIndex+MAX_PACKETS_COUNT)%MAX_PACKETS_COUNT;
